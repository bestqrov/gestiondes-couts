...
describe('generateUnitLevelExcel', () => {
  ...
  it('writes one row per physical unit, with tax columns that reconcile exactly to the source montants', async () => {
    ...
    expect(headerRow.getCell(5).value).toBe('N° Enregistrement');
    expect(headerRow.getCell(6).value).toBe('HSC');
    ...
    expect(headerRow.getCell(8).value).toBe('DTS IMPORT NORMAL');
    expect(headerRow.getCell(9).value).toBe('TVA IMPORT AUTRE PDS');
    expect(headerRow.getCell(10).value).toBe('TAXE F.P.E.I. EXP.');
    ...
    expect(firstRow.getCell(5).value).toBe('0076481 X 25/06/2026');
    expect(firstRow.getCell(7).value).toBe(1);
    // Valeur Déclarée (27147.0) / quantite (354) — same value on every row of article 1.
    expect(Number(firstRow.getCell(11).value)).toBeCloseTo(27147.0 / 354, 4);
    ...
    expect(Number(firstRow.getCell(12).value)).toBeCloseTo(27147.0 / 354 / 40039.992, 6);

    // last row of article 1, first row of article 2 resets serial number
    const lastRowArticle1 = sheet.getRow(357);
    expect(lastRowArticle1.getCell(7).value).toBe(354);
    const firstRowArticle2 = sheet.getRow(358);
    expect(firstRowArticle2.getCell(7).value).toBe(1);
    ...
    expect(Number(firstRowArticle2.getCell(11).value)).toBeCloseTo(12892.992 / 200, 4);
    // Prorata — still divided by the whole declaration's total, not article 2's own total.
    expect(Number(firstRowArticle2.getCell(12).value)).toBeCloseTo(12892.992 / 200 / 40039.992, 6);
    ...
    for (let rowNum = 4; rowNum <= 357; rowNum++) {
      ...
      sum000110 += Number(row.getCell(8).value);
      sum002109 += Number(row.getCell(9).value);
      sum007217 += Number(row.getCell(10).value);
    }
    ...
    for (let rowNum = 4; rowNum <= 557; rowNum++) {
      prorataSum += Number(sheet.getRow(rowNum).getCell(12).value);
    }
  ...
  it('fills every row\'s N° Enregistrement, Date d\'arrivée, and DONNEES COMPTABLES with the DUM\'s declaration-wide values when present', async () => {
    ...
    expect(sheet.getRow(4).getCell(5).value).toBe('0066046 E 08/07/2026');
    ...
    expect(sheet.getRow(358).getCell(5).value).toBe('0066046 E 08/07/2026');
  });
  ...
  it('zero-fills tax columns for articles that lack a given code, across the actual generator (not just the union helper)', async () => {
    ...
    // header: Nom Article | DONNEES COMPTABLES | Poids net (kg) | Date d'arrivée | N° Enregistrement | HSC | Serial Number | 000110 | 002109 | 007217 (sorted union) | Valeur Déclarée
    const headerRow = sheet.getRow(3);
    expect(headerRow.getCell(8).value).toBe('DTS IMPORT NORMAL');
    expect(headerRow.getCell(9).value).toBe('TVA IMPORT AUTRE PDS');
    expect(headerRow.getCell(10).value).toBe('TAXE F.P.E.I. EXP.');
    ...
    for (let rowNum = 4; rowNum <= 6; rowNum++) {
      const row = sheet.getRow(rowNum);
      expect(Number(row.getCell(9).value)).toBe(0); // 002109 column, article A doesn't have this code
    }
    // article A's 007217 column (montant=3 across 3 units) should reconcile to 3
    let sumA007217 = 0;
    for (let rowNum = 4; rowNum <= 6; rowNum++) {
      sumA007217 += Number(sheet.getRow(rowNum).getCell(10).value);
    }
    ...
    for (let rowNum = 7; rowNum <= 8; rowNum++) {
      const row = sheet.getRow(rowNum);
      expect(Number(row.getCell(8).value)).toBe(0); // 000110 column, article B doesn't have this code
      expect(Number(row.getCell(10).value)).toBe(0); // 007217 column, article B doesn't have this code
    }
    // article B's 002109 column (montant=10 across 2 units) should reconcile to 10
    let sumB002109 = 0;
    for (let rowNum = 7; rowNum <= 8; rowNum++) {
      sumB002109 += Number(sheet.getRow(rowNum).getCell(9).value);
    }
  ...
  it('adds a column for a RECAPITULATION rubrique that never appears in any article\'s own tax rows, filled as montant × Prorata', async () => {
    ...
    // header: Nom Article | DONNEES COMPTABLES | Poids net (kg) | Date d'arrivée | N° Enregistrement | HSC | Serial Number | 000110 | REDV.INF.(AVEC D et T) | Valeur Déclarée | Prorata
    const headerRow = sheet.getRow(3);
    expect(headerRow.getCell(9).value).toBe('REDV.INF.(AVEC D et T)');
    expect(headerRow.getCell(10).value).toBe('Valeur Déclarée');
    expect(headerRow.getCell(11).value).toBe('Prorata');
    ...
    for (let rowNum = 4; rowNum <= 8; rowNum++) {
      sumOrdonnancement += Number(sheet.getRow(rowNum).getCell(9).value);
    }
    ...
    // is (100/3 / 150) × 100.
    const firstRow = sheet.getRow(4);
    expect(Number(firstRow.getCell(9).value)).toBeCloseTo(((100 / 3) / 150) * 100, 6);
  });
});
