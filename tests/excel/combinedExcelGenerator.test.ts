...
describe('generateCombinedExcel', () => {
  ...
  it('writes a single .xlsx file containing only the Articles summary and a combined Global sheet', async () => {
    ...
    expect(globalSheet.getRow(3).getCell(2).value).toBe("Date d'arrivée");
    expect(globalSheet.getRow(3).getCell(3).value).toBe('N° Enregistrement');
    expect(globalSheet.getRow(3).getCell(4).value).toBe('HSC');
    // Same DUM-sourced values on every row of the sheet.
    expect(globalSheet.getRow(4).getCell(2).value).toBe('24/06/2026');
    expect(globalSheet.getRow(4).getCell(3).value).toBe('0076481 X 25/06/2026');
    expect(globalSheet.getRow(557).getCell(2).value).toBe('24/06/2026');
    expect(globalSheet.getRow(557).getCell(3).value).toBe('0076481 X 25/06/2026');
    ...
    expect(globalSheet.getRow(4).getCell(5).value).toBe(1); // article 1, serial 1
    expect(globalSheet.getRow(357).getCell(5).value).toBe(354); // article 1, serial 354 (last row)
    expect(globalSheet.getRow(358).getCell(5).value).toBe(1); // article 2, serial 1 (first row after article 1)
    expect(globalSheet.getRow(557).getCell(5).value).toBe(200); // article 2, serial 200 (last row)
